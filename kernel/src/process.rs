...
impl<'a> Process<'a> {
    pub fn from_elf(elf_bytes: &'a [u8], mm: &mut mm::MemoryManager) -> Self {
        let mut user_pagetable = mm::UserPageTable::new(mm).unwrap();
...
mod tests {
    ...
    fn process_load_and_execute_userland(ctx: &mut TestContext) {
        ...
        extern "C" fn exit_trap_handler(_cause: u64, syscall_number: usize) -> TrapReturnValues {
            unsafe {
                EXIT_CODE = syscall_number;
            }

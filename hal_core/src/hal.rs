use super::mm::{self, PageAlloc, PageMap, Mmu};
...
impl<P: PageMap + Mmu + 'static, I: IrqOps> Hal<P, I> {
    ...
    pub fn set_timer_handler(&self, h: TimerCallbackFn) {
        log::trace!("Hal::set_timer_handler(0x{:x})", h as u64);
    ...
    pub fn init_kpt(
        ...
        if self.kpt
            .set(ReentrantSpinlock::new(crate::mm::prefill_pagetable::<P>(r, rw, rwx, pre_allocated, allocator)?)).is_err() {
